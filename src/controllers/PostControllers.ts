...
export const createPostReelIG = async (req: CustomRequest, res: Response) => {
    try {
        ...
        const uploadurl = `POST https://graph.facebook.com/v21.0/<YOUR_APP_USERS_INSTAGRAM_USER_ID>/media?media_type=REELS&video_url=${url}&caption=${caption}&share_to_feed=${shareToFeed}&collaborators=${collaborators}&cover_url=${coverUrl}&audio_name=${audioName}&user_tags=${userTags}&location_id=${locationId}&access_token=${accessToken}`;
        const uploadRes = await axios.post(uploadurl);
